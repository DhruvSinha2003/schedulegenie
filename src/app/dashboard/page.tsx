...
export default function Dashboard() {
  ...
  const scheduleColumns = useMemo(() => {
    ...
    const sortedTasks = [...allTasks].sort((a, b) => {
      ...
      const isValidDateA =
        !isNaN(dateA.getTime()) && !/^\d+$/.test(a.day.trim());
      const isValidDateB =
        !isNaN(dateB.getTime()) && !/^\d+$/.test(b.day.trim());
    ...
    const orderedColumns = Object.values(columnsMap).sort((colA, colB) => {
      ...
      const firstTaskAIndex = sortedTasks.findIndex((task) => {
        ...
        const taskId =
          !isNaN(date.getTime()) && !/^\d+$/.test(originalDay.trim())
  ...
  const handleToggleComplete = async (
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          (await response.json().catch(() => ({}))).message ||
            "Server error updating task status."
        );
      }
      setAllTasks((prev) =>
        prev.map((t) =>
          t.taskId === taskId ? { ...t, isCompleted: newStatus } : t
        )
      );
    } catch (err: any) {
      setError("Update task error: " + err.message);
    } finally {
  ...
  const handleDeleteTask = async (taskId: string) => {
    ...
    if (!window.confirm(`Delete task: "${taskToDelete.content}"?`)) return;
    setError(null);
    ...
    try {
      ...
      setError("Delete task error: " + err.message);
    } finally {
      setPendingTaskActions((prev) => {
        const n = { ...prev };
        delete n[taskId];
  ...
  const handleSaveTask = async (taskId: string, updates: Partial<Task>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          (await response.json().catch(() => ({}))).message ||
            "Server error saving task changes."
        );
      }
      setAllTasks((prev) =>
